...
#comment model to store comments on blogs
#uses a foreign key to the User model to link comments to users 
# and a foreign key to the Blog model to link comments to blogs
class Comment(models.Model):
    user = models.ForeignKey(User, on_delete=models.CASCADE)
    blog = models.ForeignKey(Blog, on_delete=models.CASCADE, related_name="comments")
    content = models.TextField()
    created_at = models.DateTimeField(auto_now_add=True)
    is_approved = models.BooleanField(default=True)  # optional if you want moderation
